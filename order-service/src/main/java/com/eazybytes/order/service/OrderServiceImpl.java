...
import com.eazybytes.order.dto.OrderConfirmationNotification;
import com.eazybytes.order.event.OrderEventProducer;
import com.eazybytes.order.event.model.*;
import com.eazybytes.order.model.Order;
import com.eazybytes.order.model.OrderItem;
import com.eazybytes.order.repository.OrderRepository;
...
public class OrderServiceImpl implements OrderService {
    ...
    public void processPaymentSucceeded(PaymentSucceededEvent event) {
        ...
        ConfirmInventoryReservationRequest confirmRequest = ConfirmInventoryReservationRequest.builder()
                ...
                .items(order.getItems().stream()
                        .map(item -> new CartItemResponse(item.getProductId(), item.getProductName(), item.getPrice(), item.getQuantity(), item.getColor(), true))
                        .collect(Collectors.toList()))
                .build();
    ...
    @Transactional
    public void handlePaymentPendingTimeout(UUID orderId, String transactionId) {
        Order order = getOrderById(orderId);
    ...
    private String calculateTotalPrice(List<OrderItem> items) {
